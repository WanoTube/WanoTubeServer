...
async function _formatVideo(video) {
	...
	formattedDoc.url = getSignedUrl({ key: formattedDoc.url });
...
exports.getFeed = async function (req, res) {
	...
	const formattedVideos = await Promise.all(videos.map(async function (video) {
		const formattedVideo = await _formatVideo(video);
...
exports.getVideoSuggestion = async function (req, res) {
	...
	const formattedVideos = await Promise.all(videos.map(async function (video) {
		const formattedVideo = await _formatVideo(video._doc);
		return formattedVideo;
	}));
