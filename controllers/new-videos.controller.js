...
async function _checkMusic(newVideo, channelId) {
	...
	if(recognizedMusic?.recognizeResult.status.code === 0){
		console.log(recognizedMusic?.recognizeResult);
		const account = await Account.findOne({user_id: newVideo.author_id});
		handleCopyright(newVideo.title, account._id);
	}
	await Video.findOneAndUpdate(
		{ _id: newVideo._id },
